...
export class ImageBridge {
    ...
    assertWithinImageLimits(ref) {
        ...
        if (limits === undefined)
            return;
        if (ref.bytes <= limits.maxImageBytes && ref.width * ref.height <= limits.maxImagePixels)
            return;
        throw new LlmError(`image ${String(ref.attachmentId)} exceeds the deployment image limits`
            + ` (${ref.bytes} bytes, ${ref.width}x${ref.height} px; limits ${limits.maxImageBytes} bytes, ${limits.maxImagePixels} px)`, 'IMAGE_TOO_LARGE');
    }
