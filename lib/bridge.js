...
export class ImageBridge {
    ...
    assertWithinImageLimits(ref) {
        ...
        if (ref.bytes <= limits.maxImageBytes && ref.width * ref.height <= limits.maxImagePixels && withinSide)
            return;
        const side = limits.maxImageDimension === undefined
            ? 'unbounded'
            : `${limits.maxImageDimension} px per side`;
        throw new LlmError(`image ${String(ref.attachmentId)} exceeds the deployment image limits`
            + ` (${ref.bytes} bytes, ${ref.width}x${ref.height} px; limits ${limits.maxImageBytes} bytes, ${limits.maxImagePixels} px, ${side})`, 'IMAGE_TOO_LARGE');
    }
