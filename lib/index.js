...
export function apply(ctx, config) {
    ...
    const resolveApiKey = async (connection) => {
        ...
        if (value !== undefined && value.length > 0)
            return assertUsableApiKey(value, 'llm-vl-gateway', ref);
        throw new LlmError(`llm-vl-gateway: no API key for provider route "${currentProvider()}"; store ${ref} through the`
    ...
    const resolveVlApiKey = async (ref) => {
        ...
        if (value !== undefined && value.length > 0)
            return assertUsableApiKey(value, 'llm-vl-gateway', credential);
    ...
    const adapter = new VisionGatewayAdapter({ options: connectionOptions, resolveApiKey, resolveUserId }, bridge, currentDisplayName);
    ...
    let directoryFacts = initialEntry;
    /**
    ...
    const ensureRegistrationFacts = () => {
        ...
        if (deepEqualJson(next, registeredFacts) && deepEqualJson(entry, directoryFacts))
            return;
